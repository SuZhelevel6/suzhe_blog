...
1. HTTP 1.0只支持短连接而HTTP 1.1支持**长连接（Keep-Alive机制）**，管道网络传输使得用户可以不等待服务器返回数据发起多个请求，可以减少整体的响应时间。但是 服务器必须按照顺序回复消息，因此会发生队头阻塞的问题。
...
## ❗HTTP2.0与HTTP1.1的区别（多二   送）

1. 多路复用：1.1是逐个加载资源，一个资源未加载会阻塞后面的资源加载。而2.0可以同时发送多个数据流，不会阻塞其他资源的加载。
...
当用户试图通过 HTTPS 访问一个网站时，用户的浏览器首先会与网站服务器建立一个“握手”过程，在这个过程中，服务器会提供其 **SSL** **证书**，该证书包含了服务器的**公钥**。浏览器通过一系列校验确保证书有效并属于真正的网站后，会生成一个**对称加密****的密钥**，并用服务器的**公钥加密发送**给服务器。服务器收到之后用自己的私钥解密，获取对称加密密钥。此后，双方通过这个对称密钥进行加密通讯。

**防范中间人攻击**：数字证书+数字签名，首先数字证书会将证书的hash值用CA机构私钥加密，然后用公钥验证，确保公钥的正确性，然后数字签名的时候通过hash值用私钥加密的方式确保加密密钥的数据完整性。
...
**拥塞控制**：拥塞窗口，大小取决于网络拥塞程度。四种控制算法，慢开始(slow-start)、拥塞避免(congestion avoidance)、快重传(fast retransmit)和快恢复(fast recovery)，前两个算法应对拥塞，后两个应对丢失
...
1. 当浏览器从服务器上请求 web 页面时， 属于该页面的 cookie 会被添加到该请求中。服务端通过这种方式来获取用户的信息。
...
虚拟内存就是对实际的物理内存的拓展。会拓展一片虚拟空间让内存容量显得很大。
...
原理：硬件内存扩展到一个比实际物理内存更大的虚拟地址空间，将部分数据存储在硬盘上，而不是全部加载到物理内存，当程序访问被存储在虚拟地址空间中但尚未加载到物理内存的数据时，操作系统将负责将这些数据从硬盘加载到内存中。

好处：

 - 内存隔离：虚拟内存通过将每个进程的地址空间映射到独立的虚拟地址空间，实现了进程之间的内存隔离

 - 并发性：虚拟内存使得多个程序能够同时运行，每个程序都认为它在独占系统内存。

 - 高效使用物理内存：不必一次性加载全部数据到物理内存
...
反射就是通过字符串获取到类的类对象，常用Android系统中的一些API是隐藏的或受保护的，通常不直接公开给开发者使用。反射可以绕过这些限制，访问和调用这些API。比如 Android 中 StorageManager 类中的 getVolumePaths() 方法，返回储存卷的路径，在需要读取SD卡中的目录的时候就需要使用。
...
堆：用于存储**对象实例**和**数组等动态分配的****内存**，动态分配不适合用栈
