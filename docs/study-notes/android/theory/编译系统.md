...
prebuilt_apk {
    name: "APP_NAME",  // APP_NAME 应替换为自动获取的APK名称
    srcs: ["APP_NAME.apk"],
    ...
    // 启用 ProGuard 优化，并指定 proguard 配置文件
...
prebuilt_etc {
    name: "com.APP_NAME.permissions.xml",  // 权限文件名称，应替换为具体应用的 XML 文件名称

    // XML 文件路径
    src: "com.APP_NAME.permissions.xml",
...
这两个变量就是表示将应用设置为特权应用。设置为特权应用需要声明应用的权限(一般是应用的 AndroidManifest.xml里面声明的那些权限)。有以下几种方式:

1. 直接在`frameworks/base/data/etc/privapp-permissions-platform.xml`中添加，此方式不推荐
2. 在构建应用的时候在 mk 或者 bp 中添加，推荐

通过第二种方式可以让我们自己编写的com.mymenuui.permissions.xml 放置到`frameworks/base/data/etc/`目录下。
